...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c1, cond, update_emas=False, global_step=None):
        bs = c1.shape[0]
        ...
        if self.network_type == 'eg3d':
            ...
            imgs = self.G(angles=c1, ws=ws, update_emas=update_emas, nerf_init_args=nerf_init_args) # b,3,h,w
        return imgs, ws
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c1, cond, gain, cur_nimg, **kwargs):
        ...
        bs = real_c.shape[0]
        # TODO:改变loss中cond的使用方式 [ok]
        # Gmain: Maximize logits for generated images.
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, c1=gen_c1, cond=cond, global_step=cur_nimg)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c1, cond=cond, update_emas=True, global_step=cur_nimg)
