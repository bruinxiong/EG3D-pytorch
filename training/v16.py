...
class Generator(torch.nn.Module):
    ...
    def forward(self, z=None, angles=None, ws=None, truncation_psi=1, truncation_cutoff=None, 
        ...
        nerf_init_args['nerf_noise'] =  0.5 # 1.0 # 去除noise
        ...
        #     gen_low = (gen_low - gen_low.min()) / (gen_low.max() - gen_low.min()) * 2.0 - 1
    ...
    def get_sigma(self, grid_z, truncation_psi=1, truncation_cutoff=None, 
                update_emas=True, nerf_init_args={}, cond=None, **synthesis_kwargs
                ):
        nerf_init_args = {}
        nerf_init_args['num_steps'] = 128
        ...
        volume_channel = nerf_feat.shape[-1]
        h = w = img_size
        nerf_feat = nerf_feat.reshape(bs, num_steps, h, w, volume_channel).permute(0, 2, 3, 1, 4)
        sigmas = nerf_feat[..., -1]
        return sigmas
