...
class Generator(torch.nn.Module):
    ...
    def forward(self, z=None, c=None, angles=None, ws=None, truncation_psi=1, truncation_cutoff=None, 
        ...
        if ws is None:  # mappint network 不使用cond
            if angles is None:
                ...
                angles = c
            bs = angles.shape[0]
            cond = self.trans_c_to_matrix(angles).reshape(bs, 16) # bs, 16
        ...
        gen_low = F.interpolate(gen_low, size=(512, 512), mode='bilinear')
