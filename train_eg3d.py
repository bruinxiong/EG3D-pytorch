...
def subprocess_fn(rank, c, temp_dir):
    ...
    training_loop_eg3d.training_loop(rank=rank, **c)
...
def init_dataset_kwargs(data, cond_data, resolution):
    try:
        dataset_kwargs = dnnlib.EasyDict(
            class_name='training.dataset_eg3d.ImageFolderDataset', path=data, use_labels=True, max_size=None,\
                resolution=resolution,
                 xflip=False, cond_path=cond_data)
...
def main(**kwargs):
    ...
    if opts.cfg == 'stylegan2':
        ...
        raise ValueError('generator type error.')
