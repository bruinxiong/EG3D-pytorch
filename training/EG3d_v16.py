...
# 输出图像为512
# 高分辨率
# rgb feature的通道数为32 不是3
# FP16 ; D用4层
# super res模块，增加img的输入
# nerf size 从64增加到128 【x】
...
class LightDecoder(nn.Module):
    def __init__(self, in_c, mid_c, out_c):
        ...
        self.base = [
            FullyConnectedLayer(in_c, mid_c, activation='relu'),
            FullyConnectedLayer(mid_c, mid_c, activation='relu'),
            # FullyConnectedLayer(mid_c, mid_c, activation='relu'),
        ]
...
class SuperResolutionNet(nn.Module): # 简化计算量，2倍计算量
    ...
    def forward(self, x, ws, img, **block_kwargs):
        # img = None
        block_ws = []
...
# class Generator(TestModel):
    ...
    def forward(self, z=None, angles=None, ws=None, truncation_psi=1, truncation_cutoff=None, 
        ...
        nerf_init_args['img_size'] = img_size  # nerf光线数目
        nerf_init_args['fov'] = 13.6
        nerf_init_args['nerf_noise'] =  0.0 # 1.0 # 去除noise
        ...
        assert pixels_fea.shape[1] == 32
        if img_size != 128:
            pixels_fea = F.interpolate(pixels_fea, size=(128, 128), mode='bilinear')
        # pixels_fea = torch.sigmoid(pixels_fea) * 2 - 1  # -1, 1  
        # return pixels_fea
        gen_low = pixels_fea[: ,:3]
        ws = ws[:, self.synthesis.num_ws:]  # 超分辨率的ws
        gen_high = self.super_res(pixels_fea, ws.contiguous(), gen_low)  # 512
        return gen_high

        gen_low = F.interpolate(gen_low, size=(self.img_resolution, self.img_resolution), mode='bilinear')
...
class TestModel(torch.nn.Module):
    def get_volume(self, zs, cond, truncation_psi=1, truncation_cutoff=None, 
                update_emas=True,  **synthesis_kwargs
                ):
        bs = cond.shape[0]
        cond = self.trans_c_to_matrix(cond).reshape(bs, 16)
        ws = self.mapping(zs, c=cond, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        # print(ws.keys())
        backbone_feats = self.synthesis(ws[:, :self.synthesis.num_ws], update_emas=update_emas, **synthesis_kwargs)  # b,32*3,128,128
        return backbone_feats, ws
    
    def test_forward(self, backbone_feats, ws, angles, nerf_init_args={}, **synthesis_kwargs):
        img_size = nerf_init_args['img_size']
        nerf_init_args['num_steps'] = 48 if self.training else 96
        nerf_init_args['img_size'] = img_size  # nerf光线数目
        nerf_init_args['fov'] = 13.6
