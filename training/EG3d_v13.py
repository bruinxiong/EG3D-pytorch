...
# nerf noise越来越小
# 光线数目渐进增加
# decoder增加激活函数，volume render之后增加线性变化 [x]
# 更换decoder代码
# 归一化系数从0.22调整为0.18                        [x]
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, feature: torch.Tensor):
        ...
        for idx in range(self.num_layers - 1):
            ...
            x = layer(x)
        x = getattr(self, f'fc{self.num_layers - 1}')(x)  # bs_n, h, w, 33
        o = x
        # rgb, sigma = o[..., :-1], o[..., -1:]
        # rgb = torch.sigmoid(rgb)
        # o = torch.cat([rgb, sigma], dim=-1)
        return o
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z=None, angles=None, ws=None, truncation_psi=1, truncation_cutoff=None, 
        ...
        img_size = nerf_init_args['img_size']  # 渐进增加
        nerf_noise = nerf_init_args['nerf_noise'] # 渐进降低
        # print(img_size, nerf_noise)
        nerf_init_args = {}
        nerf_init_args['num_steps'] = 48
        nerf_init_args['img_size'] = img_size  # nerf光线数目
        nerf_init_args['fov'] = 13.6
        nerf_init_args['nerf_noise'] = nerf_noise # 去除noise
        ...
        nerf_feat = self.nerf_decoder(nerf_feat)  # bs*n_step, h, w, 33

        volume_channel = nerf_channel + 1
        h = w = img_size
        ...
        if True:
            ...
            fine_output  = self.nerf_decoder(fine_output)  # bs*n_step, h, w, 33
        ...
        # pixels_fea = 2 * pixels_fea - 1.0  # 值域为(-1, 1)，因为rgb feat增加了sigmoid
        if img_size != 128:
